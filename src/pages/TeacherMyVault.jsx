...
export default function TeacherMyVault() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("myvault_teacher_files");
    if (stored) setFiles(JSON.parse(stored));
  }, []);

  // Save files to localStorage on change
  useEffect(() => {
    localStorage.setItem("myvault_teacher_files", JSON.stringify(files));
  }, [files]);
  ...
  return (
    <div className="content">
      ...
      {/* ---- Vault Summary Section (UI unchanged) ---- */}
      <section className="vault-summary-section">
        <h2>Vault Summary</h2>
        <p className="vault-subtitle">
          Overview of your shared and private course materials.
        </p>
      ...
      <section className="upload-box">
        <p>Drag & Drop course materials here</p>
        <span>or</span>
        <button
          className="upload-btn"
          onClick={() => document.getElementById("fileInputTeacher").click()}
        >
          Browse Files
        </button>
